...
### Added
- **HTML 文件支持渲染预览与源码编辑切换**:`.html` / `.htm` 文件默认在隔离沙箱的预览标签中直接渲染（脚本可运行，但无法访问本地其他文件或编辑器本身），标签左上角一键切换为带语法高亮的源码编辑；切回预览前会自动保存并整页重新渲染。每个文件的预览/源码偏好按路径记忆，超过 2MB 的 HTML 直接以源码模式打开。桌面端通过新增 `local-html:` 本地协议为沙箱提供文档与静态资源；移动端仍显示占位提示。
## [0.13.241] - 2026-09-25

### Fixed
- **打开超大文本文件不再白屏卡顿**：超过 2MB 的纯文本/重文当不再用全量 `<textarea>` 渲染（实测 15MB 文件首次布局阻塞主线程约 6.6 秒），改用虚拟化的 CodeMirror 编辑器，打开与滚动全程不阻塞；保存、未保存标记、光标恢复契约与代码文件一致。
...
### Changed
- **标签页悬浮提示** — 鼠标悬浮标签约 150ms 后，在标签下方显示卡片：文件全名、完整路径、上次修改时间；未保存标签显示提示文案。替代原生命令 title。

## [0.13.236] - 2026-09-19

### Changed
- **活动栏文件/大纲/全局搜索按钮二次点击可收起侧栏** — 当对应侧栏模式已激活且侧栏展开时，再次点击同一按钮收起侧栏；再点一次重新展开。侧栏收起或处于其他模式时行为不变（切换/展开到该模式）。

## [0.13.225] - 2026-09-19

### Fixed
- **文件拖入顶栏提示层文字在浅色主题下看不清** — 拖拽悬停提示层的背景从固定黑色半透明遮罩改为跟随主题的浅色毛玻璃遮罩（带轻阴影），标题改用 `--text-strong`，在浅色与深色主题下都与遮罩保持足够对比。
...
### Added
- 附件文件夹设置（设置 → 文件与图片）：新增“插入图片时”选项，未配置图床命令时本地图片/附件可选取存放位置——复制图片到当前文件夹（./）、复制图片到 ./assets 文件夹、复制图片到 ./${filename}.assets 文件夹（Typora 风格，按文档名命名），或复制到指定路径（支持相对/绝对路径，`${filename}` 代表当前文件名）。附件文件与保存时未保存文档图片的自动迁移同遵循该设置；已配置图床命令时上传仍优先。
...
### Added
- 拖拽入废纸篓的销毁反馈：松手后被删项在原位置冻结出一行“幽灵”副本，向危险色倾变、灰化缩小下沉后消失（不依赖 watcher 刷新时机，动画必定完整播放）；投放区同步播放“吞入”脉冲并淡出，底部 toast 确认“已将 ××× 移到废纸篓”（中英文齐全）。

## [0.13.210] - 2026-09-12

### Added
- 侧边栏拖拽移到废纸篓：在工作区树中拖动文件/文件夹时，底部浮现“移到废纸篓”投放区（悬浮覆盖，不挤压树布局），拖入即调用系统废纸篓删除（与右键“移到废纸篓”同一 IPC，不弹确认，可从废纸篓恢复）。工作区根目录不可拖动，也不会被误删；中/英文文案齐全。
